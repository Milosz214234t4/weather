    ...
    for (let x = 0; x < 8; x++) {
      components.push(
        React.createElement(
          ...
          {
            style: {
              height: (plotdata[x + nextday] / maxvalue) * 50 + 10,
              background: bordercolor,
            },
            className: "plot",
            background: { bordercolor },
            onClick: () => {
              setDisplayindex(x + nextday);
            },
          },
          React.createElement(
            "legend",
      ...
      components.push(
        React.createElement(
          ...
          React.createElement(ArrowUp, {
            style: {
              rotate: winddeg[x + nextday] + "deg",
              width: (windspeed[x + nextday] / maxvalue) * 40 + 10,
              height: (windspeed[x + nextday] / maxvalue) * 40 + 10,
            },
            className: "arrow",
            onClick: () => {
              setDisplayindex(x + nextday);
